...
def get_current_admin(user: User = Depends(get_current_user)) -> User:
    """Igual que get_current_user, pero exige is_admin=True — separa el panel
    de administrador (notas de actualización) del panel del profesor."""
    if not user.is_admin:
        raise HTTPException(status_code=403, detail="Requiere una cuenta de administrador")
    return user
