...
def find_swimmer_match(db: Session, row: dict) -> MatchResult:
    ...
    if row.get("birth_date"):
        ...
        for c in candidates:
            score = fuzz.token_sort_ratio(
                ...
                _full_name(c.first_name_1, c.last_name_1)
            )
            if score >= 92:
    ...
    if full_name.strip():  # solo intenta fuzzy si al menos hay algo de nombre
        for c in all_active:
